...
library(rjson)
metadata <- fromJSON(file = "metadata.cart.2024-03-28.json")
...
mRNAMatrix <- lapply(paste('./gdc_download_20240329_012227.965163.tar',metaMatrix$file_id,metaMatrix$file_name,sep = '/'), function(fl) cleanMirFun(fl))

LIHC <- do.call("cbind", mRNAMatrix)
colnames(LIHC) <- metaMatrix$patient
table(metaMatrix$sample_type)

group <- ifelse(as.character(sapply(colnames(LIHC),function(x) substr(x,14,15)))== '01','Tumor','Normal')
write.table(LIHC,'TCGA-LIHC expdata Count.txt',sep = '\t')
